...
def spotting(result, total_gt_spot, subject_count, p, metric_final, spot_multiple, k_p, final_samples, final_dataset_spotting, 
            score_df, subject_name="", dataset_name="", final_videos="", 
            final_emotions=""):
    prev=0
    pred_subject = []
    gt_subject = []
    metric_video = MeanAveragePrecision2d(num_classes=1)
    score_plot_df = score_df.copy()
    for videoIndex, video in enumerate(final_samples[subject_count-1]):
        ...
        score_plot_df_temp = score_df.copy()
        score_plot = np.array(result[prev:prev+len(final_dataset_spotting[countVideo+videoIndex])]) #Get related frames to each video
        score_plot_df_temp.at[countVideo+videoIndex, 'Frame'] = len(score_plot)
        score_plot_df_temp.at[countVideo+videoIndex,'Score_Raw'] =  np.array(score_plot).tolist()
        score_plot_df_temp['Video'] = str(final_videos[subject_count-1][videoIndex])
        score_plot_df_temp.at[countVideo+videoIndex,'gt_peaks'] = np.array(final_samples[subject_count-1][videoIndex]).tolist()
        score_plot_df_temp.at[countVideo+videoIndex,'gt_emotions'] = np.array(final_emotions[subject_count-1][videoIndex]).tolist()

        score_plot = smooth(score_plot, k_p*2)
        score_plot_agg = score_plot.copy()
        score_plot_df = pd.concat([score_plot_df, score_plot_df_temp])
    ...
    return pred_subject, gt_subject, total_gt_spot, metric_video, metric_final, score_plot_df
