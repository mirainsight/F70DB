...
final_videos=""):
    ...
    for subject_index in range(len(final_subjects)):
        ...
        score_df = pd.DataFrame(columns=["Subject", "Video", "Frame", "Score_Raw", "gt_peaks", "gt_emotions"])
        preds, gt, total_gt_spot, metric_video, metric_final, score_df = spotting(results[0], total_gt_spot, subject_count, p, metric_final, spot_multiple, k_p, final_samples, final_dataset_spotting,score_df, subject_name=str(final_subjects[subject_count-1]), dataset_name=dataset_name, 
final_videos=final_videos, final_emotions=final_emotions)
        score_df["Subject"] = str(final_subjects[subject_count-1])
        # Create new directory for 'score_df'
        dir_score_df = dataset_name + '\\score_df\\'
        if os.path.exists(dir_score_df)==False:
            os.mkdir(dir_score_df)
        score_df.to_csv(dir_score_df+'spot_score'+str(final_subjects[subject_count-1])+ '.csv', index=False)
        temp_df = pd.read_csv(r"C:\Users\Mirac\Downloads\MEAN_Spot-then-recognize-main (1)\MEAN_Spot-then-recognize-main\CASME2\CASME2\score_df\spot_scoresALL.csv")
        temp_df = pd.concat([temp_df, score_df])
        temp_df.to_csv(r"C:\Users\Mirac\Downloads\MEAN_Spot-then-recognize-main (1)\MEAN_Spot-then-recognize-main\CASME2\CASME2\score_df\spot_scoresALL.csv", index=False)
    ...
    return TP_spot, FP_spot, FN_spot, metric_final, gt_list, pred_list, gt_tp_list, asr_score, mae_score, temp_df
